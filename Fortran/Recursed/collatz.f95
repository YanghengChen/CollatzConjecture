program collatz_conjecture 
  ...
  do num = a1, a2
    apair%num = num
    apair%sequLength = find_sequence_recursion(num)  
    i = i + 1
...
recursive function find_sequence_recursion(num) result(counter)
  integer (kind = 8), intent(in) :: num
  ...
  if (m == 1) then
    counter = 0
    return
  else if (is_even(m)) then
    counter = 1 + find_sequence_recursion(m / 2)
  else
    counter = 1 + find_sequence_recursion(3*m + 1)
  end if
end function find_sequence_recursion
